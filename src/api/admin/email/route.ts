...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const configModule = req.scope.resolve<MyConfigModule>("configModule")
  ...
  if(email){
    if(to && templateIdKey && emailTemplates[templateIdKey]){
      ...
      res.status(400).json({ result: 'check post params', body });
  } else
    res.status(400).json({ result: 'sendgrid config not found' });
